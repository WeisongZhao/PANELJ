...
public class sFRC {
	...
	/**
	 * Contains the result of a single ring from the Fourier Ring Correlation (FRC)
	 * between two images.
	 */
	public static class FRCCurveResult implements Cloneable {
		...
		/**
		 * Gets the correlation (the conjugate multiple of the two FFT images). This is
	...
	/**
	 * Contains the result of computing all the rings of the Fourier Ring
	 * Correlation (FRC) between two images.
	 */
	...
	/**
	 * Contains the Fourier Image Resolution (FIRE) result computed from the
	 * intersection of the FRC curve and a threshold curve
	 */
	...
	/**
	 * The correlation is computed using intervals around the circle circumference.
	 * The number of samples for half the circle is computed as: Pi * radius *
	 * sampling factor
	 */
	public double perimeterSamplingFactor = 1;// 1

	/**
	 * The correlation is computed using intervals around the circle circumference
	 * of the Fourier transform. The Fourier image is 2-fold radially symmetric and
	 * so the calculation can use only half the circle for speed. Note: The results
	 * will differ slightly due to the implementation of the Fourier image not being
	 * exactly symmetric and the sample points used on the perimeter not matching
	 * between the two semi-circles.
	 */
	public boolean useHalfCircle = false;
//---------------------------------PANEL Modifications------------------------------------
//---------------------------------PANEL Modifications------------------------------------
//---------------------------------PANEL Modifications------------------------------------
//---------------------------------PANEL Modifications------------------------------------				
...
//        log.status("Finished calculating FRC curve...");
	...
	public static double[] smooth(double[] d) {
		...
		for (int x = 3; x < length - 3; x++) {
			dbRt[x] = (d[x - 1] + d[x - 2] + d[x - 3] + d[x + 1] + d[x + 2] + d[x + 3] + d[x]) / 7;
		}
		dbRt[length - 2] = d[length - 2];
		dbRt[length - 1] = d[length - 1];
//		dbRt[length] = d[length];
		return dbRt;
	...
	static void computeMirrored(int size, float[] numerator, float[] absFFT1, float[] absFFT2, float[] dataA1,
		...
		// not (size-1)/2;
		// So we compute up to the centre and copy back to the other half but must not
		// miss
		// the edge pixels.
		final int centre = size / 2;
	...
	static void computeMirroredFast(int size, float[] numerator, float[] absFFT1, float[] absFFT2, float[] dataA1,
			float[] dataB1, float[] dataA2, float[] dataB2) {
		// The same as computeMirrored but ignores the pixels that are not a mirror
		// since
	...
	/**
	 * Applies a Tukey window function to the image and then pads it to the next
	 * square size power of two.
	 *
	 * @param dataImage
	...
	// Using methods to check the length should make it thread safe since we create
	 ...
	 * <p>
	 * Removed bounds checking and compute multiple values at the same time for
	 * multiple images.
	 *
	 * @param x
	 * @param y
	 ...
	 * <p>
	 * The correlation values are smoothed using a LOESS interpolation with the
	 * given parameters. If smoothing fails the original curve values are returned.
	 * If successful then the input curve is optionally copied and updated with the
	 * smoothed correlation.
	 *
	 * @param frcCurve   the frc curve
	 * @param bandwidth  the bandwidth
	 * @param robustness the robustness
	 * @param inPlace    Set to true to modify the correlation in place (do not
	 *                   create a copy)
	 ...
	 * Perform LOESS smoothing on the FRC curve
	 * <p>
	 * | * The correlation values are smoothed using a LOESS interpolation with
	 * bandwidth of 0.0707 and robustness of 0. If smoothing fails the original
	 * curve values are returned. If successful then the input curve is optionally
	 * copied and updated with the smoothed correlation.
	 *
	 * @param frcCurve the frc curve
	 * @param inPlace  Set to true to modify the correlation in place (do not create
	 *                 a copy)
	 * @return A new FRC curve
	 */
	...
	/**
	 * Calculate the curve representing the minimum correlation required to
	 * distinguish two images for each resolution in the input FRC curve.
	 *
	 * @param frcCurve
	 * @param method
	 * @return The threshold curve representing the threshold for each input spatial
	 *         frequency
	 */
	public static double[] calculateThresholdCurve(FRCCurve frcCurve, ThresholdMethod method) {
		...
		// ADH:
		// Half-Bit and 3 Sigma are explained in Supp section 5.4. However this is based
		// on
		// Heel, et al (2005) which gives a better explanation so I have updated using
		// their
		// equations.
		// Equation S.84 has an error compared to equation (13) in Heel. In fact
		// equation (17)
		// from Heel is what should be implemented for Half-bit.

		// Note: The original code used frcCurve.get(i)[2] which holds the number of
		// samples that
		// were taken from the circle. This makes the curve dependent on the number of
		// samples
		...
		// See: Heel, M. v. & Schatz, M. Fourier shell correlation threshold criteria.
		// J. Struct. Bio. 151, 250–262 (2005).
		...
		case ONE_SIGMA:
			...
			for (int i = 0; i < threshold.length; i++, nr = TWO_PI * i) {
				...
				double t = frcCurve.get(i).getNumberOfSamples();
				// Heel, Equation (2):
				// We actually want to know the number of pixels contained in the Fourier shell
				// of radius r.
				// We can compute this assuming we sampled the full circle 2*pi*r.
//				threshold[i] = sigma / Math.sqrt( nr / 2.0);
				threshold[i] = sigma / Math.sqrt(t / 2.0);
//---------------------------------PANEL Modifications------------------------------------
//---------------------------------PANEL Modifications------------------------------------
//---------------------------------PANEL Modifications------------------------------------
//---------------------------------PANEL Modifications------------------------------------				
...
//				threshold[i] = Math.min(sigma / Math.sqrt( nr / 2.0), 1);
	...
	/**
	 * Computes the crossing points of the FRC curve and the threshold curve. The
	 * intersections can be used to determine the image resolution using
	 * getCorrectIntersection(ArrayList, ThresholdMethod)
	 *
	 * @param frcCurve
	...
	public static double[][] getIntersections(FRCCurve frcCurve, double[] thresholdCurve, int max) {
		...
		for (int i = 1; i < frcCurve.getSize(); i++) {
			...
			if (x1_x2 * y3_y4 - y1_y2 * x3_x4 == 0) {
				...
				// double px = ((x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2) * (x3 * y4 - y3 *
				// x4)) /
				// ((x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4));
				// double py = ((x1 * y2 - y1 * x2) * (y3 - y4) - (y1 - y2) * (x3 * y4 - y3 *
				// x4)) /
				// ((x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4));
	...
	/**
	 * Get the correction intersection representing the image resolution. The
	 * intersection chosen depends on the method used to calculate the threshold
	 * curve using calculateThresholdCurve(double[][], ThresholdMethod)
	 * <p>
	 * The intersection corresponds the lowest spatial frequency at which there is
	 * no significant correlation between the images.
	 *
	 * @param intersections
	 * @param method
	...
	public static double[] getCorrectIntersection(double[][] intersections, ThresholdMethod method) {
		...
		default:
			if (intersections.length == 2) {
				// Choose the intersection. Just discard an intersection with a correlation
				// above 0.9
	...
	/**
	 * Utility function that calculates the Fourier Image Resolution (FIRE) number
	...
	/**
	 * Utility function that calculates the Fourier Image Resolution (FIRE) number
	 * using the provided FRC curve data.
	 *
	 * @param frcCurve
	 * @param method
...
//---------------------------------PANEL Modifications------------------------------------	
	...
	public static FIREResult calculateFire(FRCCurve frcCurve, ThresholdMethod method) {
		...
		if (intersections != null && intersections.length != 0) {
			...
			// This happens when the two split images are the same, e.g. comparing a dataset
			// to itself.
			if (perfect(frcCurve))
	...
	/**
	 * Apply spurious correlation correction using the Q-factor. Follows the method
	 * described in Niewenhuizen, et al (2013), on-line methods. This method
	 * computes a value that is subtracted from the numerator of the FRC and added
	 * to the denominator of the FRC before computing the correlation. The
	 * correlation in the FRC curve will be updated. The sums of the FRC curve are
	 * unchanged.
	 * <p>
	 * The correlation can be reset by calling this method with a Q-value of zero.
	 * <p>
	 * Note: Spurious correlation correction is only useful when computing the
	 * resolution of a single set of localisations split into two images. In this
	 * case the same emitter can be present in both images leading to spurious
	 * contribution to the correlation. Correction can be omitted providing the
	 * number of emitters is sufficiently high and the sample has spectral signal
	 * content at the computed resolution for the uncorrected curve (see
	 * Niewenhuizen, et al (2013), Nature Methods, 10, 557, Supplementary Material
	 * p.22).
	 *
	 * @param frcCurve   the frc curve
	 * @param nmPerPixel the nm per pixel in the images used to compute the FRC
	 *                   curve
	 ...
	 */
	public static void applyQCorrection(FRCCurve frcCurve, double nmPerPixel, double qValue, double mean,
			double sigma) {
		if (qValue <= 0) {
		...
		// Subtract the average residual correlation from the numerator and add to the
		// denominator
		for (int i = 1; i < frcCurve.getSize(); i++) {
	...
	/**
	 * Compute q. This is defined as 1/L, 2/L, ..., for all the spatial frequencies
	 * in the FRC curve where L is the size of the field of view. This is converted
	 * to nm using the pixel size of the input image.
	 *
	 * @param frcCurve   the frc curve
	 * @param nmPerPixel the nm per pixel in the images used to compute the FRC
	 *                   curve
	...
	/**
	 * Compute the localization PDF factor H(q) for all q. This is the integral of
	 * the distribution function of the localisation uncertainty. It is assumed to
	 * Gaussian with the specified mean and width.
	 *
	 * @param q     the q array
